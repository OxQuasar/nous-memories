...
The concrete realization is PG(2,F₂) decorated with one compass. The 0.5-bit "cosmological choice" is presentational — under full symmetry Stab(111) × Aut(Z₅), there is exactly 1 orbit. The binary choice appears only when fixing the 互 kernel line H.

The texts and the algebra are independent systems sharing a narrow interface: two distributional bridges (凶×basin, 吉×生体), 89% residual. The commentary tradition sees primes 2 and 3 but not 5.
...
- **Candidate frameworks tested and found empty:** association scheme (heterogeneous fibers), coherent configuration (generic), Walsh spectrum (automatic from ζ₅), F₁/fiber bundle (descriptive not constraining). The object doesn't have a name from existing mathematics.
...
- **Two bridges connect text to algebra:** 凶×basin (core, OR=4.25) and 吉×生体 (shell, OR=2.19). Both survive position control (CMH). Bridge orthogonality proven (R16).
...
**Q4: Why exactly two bridges?** Is it coincidence that there are exactly two, matching the two primitive projections (R5)? Or does the structure predict exactly two contact points?
