# I Ching Research — Captain's Reference

## Investigation Background

`memories/iching/trajectory.md` for the arc. `memories/iching/directory.md` for file locations.

## How to Direct

- Match method to question. The embedding pipeline is one tool, not the only tool. Different questions may need literature survey, abstract graph theory, symbolic dynamics, inverse methods, or external data.
- When the sage or coder presents a null result, reframe the question rather than closing it. A negative on one operationalization does not close the direction.
- Three epistemic categories: mechanism, coincidence, structural constraint.
- When writing findings: describe what was tested and what was observed. Do not declare questions closed or resolved. State what remains untested.

## Key Files

- Directory: `memories/iching/directory.md`
- Trajectory: `memories/iching/trajectory.md`
- Atlas: `memories/iching/atlas/atlas.json`
- Embeddings: `memories/iching/reversal/Q1/embeddings_{bge-m3,e5-large,labse,sikuroberta}.npz`
- Dynamics questions: `memories/iching/dynamics/questions.md`
