...
# Only query reserves essential for classification: ETH-correlated + major stablecoins + WBTC
KEY_RESERVES_V2 = {"WETH", "stETH", "wstETH", "USDC", "USDT", "DAI", "WBTC"}
        ...
        for (ui, ri), (ok, d) in zip(cmap, ret):
            ...
            if td > 0:
                if sym in eth_corr:
                    ...
                    info["other_debt"] += td
        bn = i // users_per_batch + 1
        if bn % 50 == 0 or bn == total_batches:
